from transformers import GPT2Tokenizer, GPT2LMHeadModel, PreTrainedTokenizerBase, PreTrainedModel
import torch
...
while True:
    ...
    while len(tokenizer.decode(input_ids[0]).split()) < target_length:
        output = model.generate(input_ids, max_length=input_ids.shape[1] + 1, do_sample=True)
